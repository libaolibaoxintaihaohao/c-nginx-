...
//分配内存
//memCount：分配的字节大小
//ifmemset：是否要把分配的内存初始化为0；         memCount ：要分配的字节大小    ifmemset：是否将分配的内存全部置为0
void *CMemory::AllocMemory(int memCount,bool ifmemset)     //分配内存
{	    
...
//内存释放函数
void CMemory::FreeMemory(void *point)    //分配内存
{		
