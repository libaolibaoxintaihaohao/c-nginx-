...
{    
    ...
    for(int i = 0; i < m_iThreadNum; ++i)               //ThreadItem装着线程的相关信息，比如线程的id等，把他传给别人，拿到他的id就可以让id做事，比如打印日志，发送消息等等
    {
        m_threadVector.push_back(pNew = new ThreadItem(this));             //创建 一个新线程对象 并入到容器中     this 是当前对象的指针  因为这段代码在 CThreadPool::Create() 函数内部，所以 this 指向正在调用 Create() 的那个 CThreadPool 对象。     
        err = pthread_create(&pNew->_Handle, NULL, ThreadFunc, pNew);      //创建线程，错误不返回到errno，一般返回错误码
        if(err != 0)          //_handle保存着线程id系统分配 的线程标识符
        {
            //创建线程有错
...
void* CThreadPool::ThreadFunc(void* threadData)
{
    ...
    ThreadItem *pThread = static_cast<ThreadItem*>(threadData);     //threadData是ThreadItem类型的，就是上面这一句pNew = new ThreadItem(this)，threadData 是 void*，所以static_cast<ThreadItem*>(threadData)：把它转回正确的类型
    CThreadPool *pThreadPoolObj = pThread->_pThis;             //pThread 访问它的成员 _pThis，而 _pThis 正是当初创建 ThreadItem 时传入的 this，这个this指向调用create()的线程对象，所以this指向正在调用create()的线程对象。
    ...
    while(true)
    {
        ...
        while ( (pThreadPoolObj->m_MsgRecvQueue.size() == 0) && m_shutdown == false)    //无任务 且 未要求退出    任务队列是空的，所以用pthread_cond_wait(&m_pthreadCond, &m_pthreadMutex);来阻塞线程
        {
            if(pThread->ifrunning == false)            
                pThread->ifrunning = true; //标记为true了才允许调用StopAll()：测试中发现如果Create()和StopAll()紧挨着调用，就会导致线程混乱，所以每个线程必须执行到这里，才认为是启动成功了；
            pthread_cond_wait(&m_pthreadCond, &m_pthreadMutex); //整个服务器程序刚初始化的时候，所有线程必然是卡在这里等待的；      原子地释放互斥锁 m_pthreadMutex  ，   线程进入等待状态，挂起在条件变量 m_pthreadCond 上，当其他线程调用 pthread_cond_signal() 或 pthread_cond_broadcast() 时被唤醒
        }
