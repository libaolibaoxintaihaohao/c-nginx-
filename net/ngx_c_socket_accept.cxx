...
void CSocekt::ngx_event_accept(lpngx_connection_t oldc)
{
    ...
    socklen = sizeof(mysockaddr);               //初始化 socklen，用于 accept 函数获取客户端地址长度。
    do   //用do，跳到while后边去方便
    {     
        if(use_accept4)
        {
        ...
        }             //监听socket的fd       &mysockaddr存放客户端地址的结构体  第三个参数是&mysockaddr的长度        
        else
        {
            //以为listen套接字是非阻塞的，所以即便已完成连接队列为空，accept()也不会卡在这里；
            s = accept(oldc->fd, &mysockaddr, &socklen);                        //accept() 是服务器从“内核已完成连接队列”中取出一个新客户端连接，并返回一个新的 socket 文件描述符（fd），用于后续与该客户端通信。     
        }
        ...
        if(s == -1)
        {
            ...
            if(err == EAGAIN) //accept()没准备好，这个EAGAIN错误EWOULDBLOCK是一样的                  eagin没链接可取了
            {
                //除非你用一个循环不断的accept()取走所有的连接，不然一般不会有这个错误【我们这里只取一个连接，也就是accept()一次】
                return ;
            } 
            level = NGX_LOG_ALERT;    //NGX_LOG_ALERT：严重系统问题（默认级别）
        ...
        //走到这里的，表示accept4()/accept()成功了         //m_connectionList.size()	连接池总容量（包括：空闲 + 使用中 + 待回收）         m_freeconnectionList.size()	当前真正空闲、可复用的连接数                m_worker_connections * 5	经验阈值（如 2048 * 5 = 10240）
        if(m_onlineUserCount >= m_worker_connections)  //用户连接数过多，要关闭该用户socket，因为现在也没分配连接，所以直接关闭即可    第一道防线：限制最大在线用户数
        {
        ...
        if(m_connectionList.size() > (m_worker_connections * 5))        //m_connectionList.size()总连接数      m_worker_connections * 5   含义：最大连接数*5  
        {
            //比如你允许同时最大2048个连接，但连接池却有了 2048*5这么大的容量，这肯定是表示短时间内 产生大量连接/断开，因为我们的延迟回收机制，这里连接还在垃圾池里没有被回收
            if(m_freeconnectionList.size() < m_worker_connections)         //m_freeconnectionList.size()空闲连接数
            {
        ...
        newc->whandler = &CSocekt::ngx_write_request_handler; //设置数据发送时的写处理函数。          绑定读写回调函数
        ...
        */
        ...
        if(m_ifkickTimeCount == 1)        //如果开启了踢人功能
        {
            AddToTimerQueue(newc);           //把新连接加入到定时器容器中，开始计时【注意：这里是加入到定时器容器中，并不是说马上就计时踢人，而是从现在开始算起，如果在心跳时间内没有任何数据来，则踢人】
        }
