...
{  
    ...
    ssize_t reco = recvproc(pConn,pConn->precvbuf,pConn->irecvlen);           //pConn->precvbuf：当前要写入数据的内存地址  pConn->irecvlen：当前要接收的数据长度
    if(reco <= 0)  
    {
...
ssize_t CSocekt::recvproc(lpngx_connection_t pConn,char *buff,ssize_t buflen)  //ssize_t是有符号整型，在32位机器上等同与int，在64位机器上等同与long int，size_t就是无符号型的ssize_t
{
    ...
    if(n == 0)
    {
        ...
        zdClosesocketProc(pConn);        //“彻底、安全地把一个客户端连接关掉，并把它用过的资源收拾干净，准备 reuse（复用）。”
        return -1;
    }
    //客户端没断，走这里 
    if(n < 0) //这被认为有错误发生
