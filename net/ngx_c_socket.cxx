...
{
    //发消息互斥量初始化
    if(pthread_mutex_init(&m_sendMessageQueueMutex, NULL)  != 0)                  //m_sendMessageQueueMutex	：发送消息队列
    {        
        ngx_log_stderr(0,"CSocekt::Initialize_subproc()中pthread_mutex_init(&m_sendMessageQueueMutex)失败.");
        return false;    
    }
    //连接相关互斥量初始化
    if(pthread_mutex_init(&m_connectionMutex, NULL)  != 0)                       //m_connectionMutex	全局连接表（如 std::map<fd, conn>）
    {
    ...
    {
        ngx_log_stderr(0,"CSocekt::Initialize_subproc()中pthread_mutex_init(&m_recyconnqueueMutex)失败.");             //m_recyconnqueueMutex	待回收连接队列
        return false;    
    } 
    //和时间处理队列有关的互斥量初始化
    if(pthread_mutex_init(&m_timequeueMutex, NULL)  != 0)
    {
        ngx_log_stderr(0,"CSocekt::Initialize_subproc()中pthread_mutex_init(&m_timequeueMutex)失败.");                 //m_timequeueMutex	定时器队列（记录 lastPingTime 的连接）
        return false;    
    }
    ...
    if(sem_init(&m_semEventSendQueue,0,0) == -1)               //第一个参数：信号量变量地址           第二个参数 = 0：表示在线程间共享（不是进程间）      第三个参数 = 0：初始值为 0 → sem_wait() 会阻塞
    {
    ...
    err = pthread_create(&pSendQueue->_Handle, NULL, ServerSendQueueThread,pSendQueue); //创建线程，错误不返回到errno，一般返回错误码         &pSendQueue->_Handle：用来保存新线程的 ID
    if(err != 0)
    {
...
{    
    ...
    {        
        ...
        if(setsockopt(isock,SOL_SOCKET, SO_REUSEADDR,(const void *) &reuseaddr, sizeof(reuseaddr)) == -1)//作用是设置：即使这个 IP+端口最近被用过（比如程序刚重启），也允许我立即绑定它！
        {
        ...
        {                                                                                            //这段代码的作用是：尝试启用 SO_REUSEPORT 选项，以支持多个进程或线程同时监听同一个 IP+端口，从而避免“惊群”问题并提升多核并发性能。
            //失败就失败吧，失败顶多是惊群，但程序依旧可以正常运行，所以仅仅提示一下即可                       底层：作用是让每个线程/进程拥有自己独立的 listen socket，而不是共享同一个
        ...
        iport = p_config->GetIntDefault(strinfo,10000);    //在配置项列表中查找名为 "ListenPort0" 的项；如果没找到就返回10000
        serv_addr.sin_port = htons((in_port_t)iport);   //in_port_t其实就是uint16_t         把小端转成大端方便网络传输使用
                            //转换的是端口号
        //绑定服务器地址结构体
