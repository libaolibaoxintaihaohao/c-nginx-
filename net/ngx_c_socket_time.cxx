...
time_t CSocekt::GetEarliestTime()        //获取定时器队列中最早（最小）的超时时间戳
{
...
void CSocekt::DeleteFromTimerQueue(lpngx_connection_t pConn)             //从定时器队列（m_timerQueuemap）中删除所有与指定连接 pConn 关联的定时器项，并释放其内存。
{
