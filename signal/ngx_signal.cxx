...
int ngx_init_signals()
{
    ...
    for (sig = signals; sig->signo != 0; sig++)  //将signo ==0作为一个标记，因为信号的编号都不为0；  意义是：从全局数组 signals 开始，逐个处理每个信号，直到遇到 signo == 0（表示数组结束）。
    {        
        //我们注意，现在要把一堆信息往 变量sa对应的结构里弄 ......  依旧是清零
        ...
        else
        {
            sa.sa_handler = SIG_IGN; //sa_handler:这个标记SIG_IGN给到sa_handler成员，表示忽略信号的处理程序，否则操作系统的缺省信号处理程序很可能把这个进程杀掉；
                                    ...
                                    //初始化一个空的信号集，表示 在执行信号处理函数时不阻塞任何其他信号。
