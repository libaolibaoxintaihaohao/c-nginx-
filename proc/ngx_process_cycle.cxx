...
{    
    ...
    //首先我设置主进程标题---------begin
    size_t size;            //用于计算最终要设置的进程标题字符串的总长度
    int    i;
    size = sizeof(master_process);  //注意我这里用的是sizeof，所以字符串末尾的\0是被计算进来了的
    size += g_argvneedmem;          //argv参数长度加进来           g_argvneedmem表示原始命令行参数总长度（来自 main()）。   
    if(size < 1000) //长度小于这个，我才设置标题
    {
        char title[1000] = {0};
        strcpy(title,(const char *)master_process); //"master process"   函数作用是将后面的内容复制到前面的数组中
        strcat(title," ");  //跟一个空格分开一些，清晰    //"master process "
        for (i = 0; i < g_os_argc; i++)         //"master process ./nginx"   argc表示命令行参数个数
        {
             //if (i > 0) strcat(title, " ");     //这里应该再加上空格，让每个参数之间分开          argv表示命令行参数数组
            strcat(title,g_os_argv[i]);
        }//end for
        ngx_setproctitle(title); //设置标题     这里
...
static int ngx_spawn_process(int inum,const char *pprocname)
{
    ...
    case 0:  //子进程分支
        ngx_parent = ngx_pid;              //因为是子进程了，所有原来的pid变成了父pid    ngx_pid是全局变量，原为主进程的 PID
...
static void ngx_worker_process_init(int inum)
{
    ...
    sigemptyset(&set);  //清空信号集
    if (sigprocmask(SIG_SETMASK, &set, NULL) == -1)  //原来是屏蔽那10个信号【防止fork()期间收到信号导致混乱】，现在不再屏蔽任何信号【接收任何信号】，接触信号屏蔽
    {
