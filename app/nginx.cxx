...
{     
    ...
    if(p_config->Load("nginx.conf") == false) //把配置文件内容载入到内存  没加载成功就进行下一步操作初始化日志  没成功的话就说明已经把 nginx.conf加载到缓冲区buf上了，之后就可以直接用Getstring获得配置里的内容     
    {   
        ngx_log_init();    //初始化日志
    ...
    if(g_socket.Initialize() == false)//初始化socket       包括一系列的socket创建，地址绑定等等，最后把监听客户端的socket的结构体都放进了一个list里用来管理
    {
        exitcode = 1;
        goto lblexit;
    }
    ...
    {
        //1：按守护进程方式运行
        int cdaemonresult = ngx_daemon();//调用 ngx_daemon() 创建守护进程
        if(cdaemonresult == -1) //fork()失败
        {
            exitcode = 1;    //标记失败
            goto lblexit;
        }
        if(cdaemonresult == 1)//等于1表示当前是原始的父进程，应当立即退出
        {
            //这是原始的父进程
