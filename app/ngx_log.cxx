...
{    
    ...
    u_char  errstr[NGX_MAX_ERROR_STR+1]; //缓冲区   2048  -- ************  +1是我自己填的，感谢官方写法有点小瑕疵，所以动手调整一下      缓冲区，用于组装最终要输出的完整错误字符串
    u_char  *p,*last;    //p当前写入位置指针  last缓冲区末尾指针

    memset(errstr,0,sizeof(errstr));     //缓冲区重置  我个人加的，这块有必要加，至少在va_end处理之前有必要，否则字符串没有结束标记不行的；***************************
    ...
    *p++ = '\n'; //增加个换行符    
    ...
    if(ngx_log.fd > STDERR_FILENO) //如果这是个有效的日志文件，本条件肯定成立，此时也才有意义将这个信息写到日志文件
    {
        ...
        ngx_log_error_core(NGX_LOG_STDERR,err,(const char *)errstr); //将存储在 errstr 中的错误消息按照 NGX_LOG_STDERR 所指定的方式（即输出到标准错误）进行记录，并可能附加由 err 提供的额外错误信息
    }    
    return;
}
...
void ngx_log_error_core(int level,  int err, const char *fmt, ...)
{
    ...
    memset(errstr,0,sizeof(errstr));//清空这个buffer  
    ...
    va_start(args, fmt);                     //使args指向起始的参数
    p = ngx_vslprintf(p, last, fmt, args);   //把fmt和args参数弄进去，组合出来这个字符串    将时间拷贝到主缓冲区
    va_end(args);                            //释放args 
...
void ngx_log_init()
{
    ...
    //从配置文件中读取和日志相关的配置信息
    CConfig *p_config = CConfig::GetInstance();//调用 CConfig 的 单例模式 接口（确保全局只有一个配置实例
    plogname = (u_char *)p_config->GetString("Log");//从配置对象中读取键为 "Log" 的字符串值
    if(plogname == NULL)
    {
        //没读到，就要给个缺省的路径文件名了
    ...
    if (ngx_log.fd == -1)  //如果有错误，则直接定位到 标准错误上去              如果 open() 失败（返回 -1），说明日志文件无法打开
    {
